...
describe AuthorizationsController, type: :request do
  context "#callback" do
    let(:fb_auth) { get "http://#{Setting.host}/authorizations/facebook/callback", nil, "omniauth.auth" => omniauth_mock(:facebook) }
    let(:google_auth) { get "http://#{Setting.host}/authorizations/google_oauth2/callback", nil, "omniauth.auth" => omniauth_mock(:google_oauth2) }
    it "success" do
      expect {
        fb_auth
